...
void run_bench(MCTS& mcts, PLY_TYPE depth) {

    // Save these variables to reset them after the bench has been run.
    PLY_TYPE old_max_depth = mcts.max_depth;
    uint64_t old_max_time = mcts.max_time;
    uint64_t old_max_nodes = mcts.max_nodes;

    Position& position = mcts.position;
    mcts.max_depth = depth;
    mcts.max_nodes = MAX_ITERATIONS;


    // Calculate the starting time
    auto start_time = std::chrono::high_resolution_clock::now();
    mcts.start_time = std::chrono::duration_cast<std::chrono::milliseconds>
            (std::chrono::time_point_cast<std::chrono::milliseconds>(start_time).time_since_epoch()).count();
    mcts.max_time = TIME_INF;
    ...
    // Reset variables
    mcts.max_depth = old_max_depth;
    mcts.max_time = old_max_time;
    mcts.max_nodes = old_max_nodes;
